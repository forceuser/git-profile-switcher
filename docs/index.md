# Docs Index

Start here before implementation files.

## Start Here

- [Backlog](./backlog.md) - current delivery status and next increments.
- [Architecture](./architecture/index.md) - system map and module boundaries.
- [CLI and TUI](./features/cli-and-tui.md) - user-facing command and terminal flows.
- [Shell Prompt](./features/shell-prompt.md) - prompt integration behavior.
- [Release With GitHub Actions](./operations/release-with-github-actions.md) - tag-based
  public npm publication flow.

## Code Discoverability

1. Open the relevant section in `docs/`.
2. Open the matching module README in `runtime/*/README.md`.
3. Then jump into implementation files.
